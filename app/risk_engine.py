...
def build_risk_plan(
    *,
    ...
    stop_dist = max(atr_value * sl_atr_mult, entry * 0.0005)
    ...
    position_leverage_mult = _getf(cfg, "POSITION_LEVERAGE_MULT", 2.0)
    position_value = nav_usd * position_leverage_mult

    max_position_value = _getf(cfg, "MAX_POSITION_VALUE_USD", position_value)
    position_value = min(position_value, max_position_value)

    qty = (position_value / entry) * vol_scale

    risk_usd = abs(qty * stop_dist)
    risk_pct = (risk_usd / nav_usd) * 100.0 if nav_usd > 0 else 0.0
